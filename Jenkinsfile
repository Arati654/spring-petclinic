...
pipeline {
    ...
    stages {
        stage('Checkout From Git') {
            steps {
                git branch: 'main', url: 'https://github.com/Arati654/spring-petclinic.git'
            }
        }
            ...
            steps {
                withSonarQubeEnv('sonarserver') {
                    sh '''
                        ...
                        -Dsonar.host.url=http://172.190.115.144:9000 \
            ...
            steps {
                ...
                sh '''
                    trivy image --format table --severity HIGH,CRITICAL \
                        --output trivy-report.txt aratiegistry.azurecr.io/${ImageName}:${BUILD_TAG}
                '''
            }
            post {
                always {
                ...
                withCredentials([
                    ...
                    script {
                        ...
                        sh '''
                            ...
                            az acr login --name aratiegistry
                            docker push aratiegistry.azurecr.io/${ImageName}:${BUILD_TAG}
                        '''
                    }
                }
            }
        ...
        stage('Deploy to Kubernetes') {
            steps {
                script {
                    ...
                    sh '''
                        az aks get-credentials --resource-group JenkinsRG --name arati-aks-cluster
